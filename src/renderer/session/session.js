(function () {
  ...
  const adapter = {
    ...
    derive: async (secret, salt) => window.JCSecure.unb64(await jc.invoke('jc:derive', window.JCSecure.b64(secret), window.JCSecure.b64(salt))),
    onAuthenticated: (auth) => jc.invoke('jc:computer-seen', id, { mac: auth.mac, services: auth.services }).catch(() => {}),
    onProgress: (fn) => jc.on('jc:progress', fn),
